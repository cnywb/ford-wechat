...
angular.module('angular-datetimepicker', []).directive('jrgDatetimepicker', [function () {
    ...
    function convertTimezone(dateMoment, tzFromMinutes, tzToMinutes, params) {
       ...
       // var dateFormatted =dateMoment.format('YYYY-MM-DD HH:mm:ss');
        var dateFormatted =dateMoment.format('YYYY-MM-DD');		//temporary string that will be used to form the final moment date object AFTER timezone conversion is done (since doesn't seem to be a way to change the timezone on an existing moment date object.. - if there was, we wouldn't need this entire function at all!)
       ...
       // ret.date =moment(dateFormatted, 'YYYY-MM-DD HH:mm:ss');
        ret.date =moment(dateFormatted, 'YYYY-MM-DD');
    ...
    return {
        ...
        template: function(element, attrs) {
            ...
            var skipAttrs =['jrgDatetimepicker', 'ngModel', 'label', 'type', 'placeholder', 'opts', 'name', 'validate', 'ngChange', 'ngClick','value'];
            ...
            if(type =='pikaday') {
                html +="<input class='jrg-datetimepicker-input' style='padding-left:15px;border:solid 1px #cfdadd;border-radius: 2px;' type='datetime' placeholder='"+attrs.placeholder+"' value='"+attrs.value+"'"+customAttrs+" ";		//NOTE: do NOT use ng-model here since we want the displayed value to potentially be DIFFERENT than the returned (ngModel) value
                if(attrs.ngClick) {
                ...
                html +="<input class='jrg-datetimepicker-input' style='padding-left:15px;border:solid 1px #cfdadd;border-radius: 2px;' type='datetime-local' placeholder='"+attrs.placeholder+"' "+customAttrs+" ";		//NOTE: do NOT use ng-model here since we want the displayed value to potentially be DIFFERENT than the returned (ngModel) value (this especially breaks iOS native datetime input display)		//UPDATE for iOS7 - need to use 'datetime-local' since 'datetime' input type is no longer supported..
                if(attrs.ngClick) {
                    html +="ng-click='ngClick()' ";
                }
                html +="/>";
        ...
        link: function(scope, element, attrs) {
            ...
            if(attrs.type =='forge') {
                ...
                if(scope.ngModel) {
                    ...
                    var modelFormatted =moment(scope.ngModel, scope.opts.formatModel).format(dateFormat+' '+timeFormat);
