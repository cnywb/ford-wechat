 ...
 *
 *
 *           页面导入这个组件时，直接引入该文件，不要导入packed.js文件，避免与修改后的文件内如不符
 *
 *           使用方式：
 *           1.为textarea定义个id属性，然后在弹出的模态窗口渲染完成的方法里面实例化。
 *
 *           instance.rendered.then(function () {
            //必须在页面渲染完成后才能初始化富文本编辑器。在controller当中通过全局函数获取实例
           //必须在页面渲染完成后才能初始化富文本编辑器。在controller当中通过全局函数获取实例
            $window.setTimeout(function () {
                new $window.TINY.editor('contentEditor', {
                    id: 'contentEditor', // (required) ID of the textarea
                    width: '100%', // (optional) width of the editor
                    height: 300, // (optional) heightof the editor
                    cssclass: 'te height-300', // (optional) CSS class of the editor
                    controlclass: 'tecontrol', // (optional) CSS class of the buttons
                    rowclass: 'teheader', // (optional) CSS class of the button rows
                    dividerclass: 'tedivider', // (optional) CSS class of the button diviers
                    controls: ['bold', 'italic', 'underline', 'strikethrough', '|', 'subscript', 'superscript', '|',
                        'orderedlist', 'unorderedlist', '|' ,'outdent' ,'indent', '|', 'leftalign', 'centeralign',
                        'rightalign', 'blockjustify', '|', 'unformat', '|', 'undo', 'redo', 'n', 'font', 'size', 'style',
                    ...
                    content: angular.isUndefined(source) ? "" : source['content'], // (optional) set the starting content else it will default to the textarea content
                    bodyid: 'editor', // (optional) attach an ID to the editor body
                    footerclass: 'tefooter', // (optional) CSS class of the footer
                    toggle: {text: '源码', activetext: '标准', cssclass: 'toggle'}, // (optional) toggle to markup view options
