        ...
        bindSelectByJSON: function ($scope, categoryCode, sourceObject, dictName) {
            this.bindByJSON($scope, categoryCode, function (dictS) {
                ...
                angular.forEach(dictS, function (dict) {
                    //多选下拉框恢复时,传入的默认值必须为数组,且数组当中的元素为对象并包含一个key为value的属性
                    if (angular.isArray(sourceObject[dictName])) {
                        angular.forEach(sourceObject[dictName], function (o) {
                            if (o["value"] == dict["value"]) {
                                dictObjArray.push(dict);
                            }
                        });
                    } else if (dict['value'] == sourceObject[dictName]) {
                        dictObj = dict;
                    }
                });
                sourceObject[dictName] = dictObj || dictObjArray;
            })
        },
        /**
...
dbUtils.factory("dbUtils", ["$modal", "$http", "$window", 'toaster', function ($modal, $http, $window, toaster) {
    ...
    function fmoney(s) {
        ...
        sEnd = sEnd || '.00';
        return sStart + (!t.split("").reverse().join("") ? "" : t.split("").reverse().join("")) + sEnd;
    }
    ...
    function getPathContext(){
        ...
        console.log(pathname.substr(0,indexNext));
    ...
    return {
        ...
        postExcel: function (transCode, reqBody, success, error) {
            Metronic.startPageLoading();
            var ApiRequest = {};
            ApiRequest["transCode"] = transCode;
            ApiRequest["requestBody"] = reqBody;
        ...
        get: function (transCode, reqBody, error) {
            ...
            ApiRequest["transCode"] = transCode;
            ApiRequest["requestBody"] = reqBody;
            $http.get("../api.ctl", ApiRequest).success(function (data) {
                Metronic.stopPageLoading();
                if (data.status == "401") {//用户未登录
                    $window.location.href = "login.html";
                } else if (data.status == "403") {//用户没有权限
        ...
        timeFormat: function (date) {
            ...
            var month = date.getMonth() + 1;
            var date = date.getDate();
            return year + "-" + (month > 9 ? month : ("0" + month)) + "-" + (date > 9 ? date : ("0" + date)) + " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
        },
        getNextDay: function (date, num) {
        ...
        getUserInfo: function () {
            var user = {};
            user["loginName"] = $window.sessionStorage.getItem("user_loginName");
            user["name"] = $window.sessionStorage.getItem("user_name");
            user["willPassDays"] = $window.sessionStorage.getItem("user_willPassDays");
            return user;
        },
        /**
         * 获取两个日期的天数
