...
function DbDataTableController($scope, dbUtils, dbImService) {
    ...
    angular.forEach($scope.dbDataTable.tableHeaders, function (item) {
        ...
        if (item.dropDownDataSource && item.dropDownItem && item.dataType === "select") {
            ...
            dropDownFields.push(item.field);
    ...
    $scope.addData = function () {
        ...
        angular.forEach(dropDownFields, function (field) {
            persistentRecord[field] = persistentRecord[field] ? persistentRecord[field].value : "";
        });
        ...
        if ($scope.dbDataTable.distinct) {
            ...
            if (flag) {
                dbUtils.warning($scope.dbDataTable.distinct.msg);
                return;
            }
        }
        if ($scope.dbDataTable.beforeAdd) {
            var flag = $scope.dbDataTable.beforeAdd(angular.copy(persistentRecord));
