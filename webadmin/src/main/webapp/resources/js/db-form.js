    ...
    return {
        ...
        controller: ['$scope', 'dbImService', function ($scope, dbImService) {
            ...
            $scope.dbForm.submit = function (isValid) {
                ...
                if (isValid) {
                    var reqBody = $scope.dbForm.getFormData();
                    ...
                    dbUtils.confirm("确定要执行此操作?", function () {
                        if ($scope.dbForm.events.onSubmit) {
                            $scope.dbForm.events.onSubmit(reqBody);
                        } else {
                            $scope.dbForm.submitting = true;
                            dbUtils.post($scope.dbForm.settings.transCode, reqBody, function (data) {
                                $scope.dbForm.submitting = false;
