 ...
 * @since 1.0
 */
@Service("browseMenuToDBLauncher")
public class BrowseMenuToDBTimer {

    private static final Logger logger = LoggerFactory.getLogger(BrowseMenuToDBTimer.class);

    @Resource(name = "browseMenuToDBJob")
    private Job job;
    ...
    public void createBatch() {
        ...
        batchTask.setBatchType(BatchType.BROWSE_MENU_TO_DB);
     ...
     */
    public void execute() {
        List<BatchTask> list = this.batchTaskService.getByType(BatchType.BROWSE_MENU_TO_DB);

        if (CollectionUtils.isEmpty(list)) {
            logger.info("本次无菜单点击记录任务！");
            return;
        }

        for (BatchTask batchTask : list) {
            String yeasterday = batchTask.getParams();
            JobParametersBuilder builder = new JobParametersBuilder();
            builder.addLong(Parameter.TASK_ID, batchTask.getId());
            builder.addString(Parameter.DATE, yeasterday);
