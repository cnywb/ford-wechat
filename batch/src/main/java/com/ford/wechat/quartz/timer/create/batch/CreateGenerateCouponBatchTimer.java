 ...
 */
@Slf4j
@Service("createGenerateCouponBatchTimer")
public class CreateGenerateCouponBatchTimer {
    ...
    public void execute() {
        ...
        //批次备注
        String remark = DateUtil.formatDate(new Date(),DateUtil.FORMAT_DATETIME_DEFAULT)+"-" + BatchType.GENERATE_COUPON.getName();
        batchTask.setRemark(remark);
        //批次类型
        batchTask.setBatchType(BatchType.GENERATE_COUPON);
        this.batchTaskService.save(batchTask);
        log.info("创建任务 代金券生成任务创建结束");
    }
