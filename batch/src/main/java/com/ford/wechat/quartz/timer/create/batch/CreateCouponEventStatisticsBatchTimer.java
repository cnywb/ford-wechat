 ...
 */
@Slf4j
@Service("createCouponEventStatisticsBatchTimer")
public class CreateCouponEventStatisticsBatchTimer {
    ...
    public void execute() {
        ...
        Date yesterday = DateUtil.getNextDay(new Date(), -1);
        ...
        //批次备注
        String remark = DateUtil.formatDate(new Date(),DateUtil.FORMAT_DATETIME_DEFAULT)+"-" + BatchType.COUPON_EVENT_STATISTICS.getName();
        batchTask.setRemark(remark);
        //批次类型
        batchTask.setBatchType(BatchType.COUPON_EVENT_STATISTICS);
        this.batchTaskService.save(batchTask);
        log.info("创建任务 认证激励统计任务创建结束");
    }
