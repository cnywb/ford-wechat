 ...
 */
@Slf4j
@Service("createCouponSendDmsBatchTimer")
public class CreateCouponSendDmsBatchTimer {
     ...
     */
    public void execute() {
        log.info("创建任务 开始创建认证激励代金券下发DMS任务");
        ...
        //批次备注
        String remark = DateUtil.formatDate(new Date(),DateUtil.FORMAT_DATETIME_DEFAULT)+"-" + BatchType.COUPON_SEND_DMS.getName();
        batchTask.setRemark(remark);
        //批次类型
        batchTask.setBatchType(BatchType.COUPON_SEND_DMS);
        this.batchTaskService.save(batchTask);
        log.info("创建任务 认证激励代金券下发DMS任务创建结束");
    }
