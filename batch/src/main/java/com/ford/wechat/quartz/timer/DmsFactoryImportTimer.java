 ...
 * All rights reserved. 2017-06-09 19:08
 * </p>
 *
 * @author MaoJiaWei
 * @version 1.0
 */
@Service("dmsFactoryJobLauncher")
public class DmsFactoryImportTimer {

    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(AutoAuthTimer.class);

    @Resource(name="dmsFactoryImportJob")
    private Job job;
     ...
     * dms主机厂数据xml 存放目录
     */
    @Value("#{settings['dms.factory.import.path']}")
    ...
    public void execute() {
        ...
        List<BatchTask> list =this.batchTaskService.getByType(BatchType.DMS_FACTORY_IMPORT);
        ...
        for(BatchTask batchTask:list){
            ...
            builder.addString(Parameter.INPUT_FILE_PATH,inputFilePath);
