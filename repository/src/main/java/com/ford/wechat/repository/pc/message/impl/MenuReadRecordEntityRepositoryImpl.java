 ...
 * All rights reserved.
 * MenuReadRecordEntityRepositoryImpl.java
 */
...
/**
 * 描述：MenuReadRecordEntityRepositoryImpl 仓储服务接口实现层代码类
 *
 * @author ziv
 * @since 1.0
 */
@Repository
public class MenuReadRecordEntityRepositoryImpl extends DefaultJpaRepository<MenuReadRecordEntity, Long> implements MenuReadRecordEntityRepository {

    @Override
    public Page pagingBy(GridPage page, String datePattern, Date startDate, Date endDate) {
        StringQuery query = StringQuery.newQuery()
                .query("select a.MENU_ID,a.MENU_NAME,a.MENU_SORT,TO_CHAR(a.READ_TIME,'" + datePattern + "'),count(1) from PC_MENU_READ_RECORD a where 1 = 1 ")
                ...
                .predicate(true)
                .query(" group by a.MENU_ID,a.MENU_NAME,a.MENU_SORT,TO_CHAR(a.READ_TIME,'" + datePattern + "') order by TO_CHAR(a.READ_TIME,'" + datePattern + "') DESC").build();
        StringQuery countQuery = StringQuery.newQuery()
                .query("select count(1) as aaaaaa from (select a.MENU_ID,a.MENU_NAME,a.MENU_SORT,TO_CHAR(a.READ_TIME,'" + datePattern + "'),count(1) from PC_MENU_READ_RECORD a where 1 = 1 ")
                .predicateNotNull(startDate)
                .query(" and a.READ_TIME >= :startDate")
                .param("startDate", startDate)
                .predicateNotNull(endDate)
                ...
                .param("endDate", endDate)
                .predicate(true)
                .query(" group by a.MENU_ID,a.MENU_NAME,a.MENU_SORT,TO_CHAR(a.READ_TIME,'" + datePattern + "'))").build();
